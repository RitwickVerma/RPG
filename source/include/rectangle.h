...
public:
    ...
    {
        return side == side::LEFT ? this->getLeft() : side == side::RIGHT ? this->getRight() : side == side::TOP ? this->getTop() : side == side::BOTTOM ? this->getBottom() : side::NONE;
    }
    ...
    void setCenter(xyfpair xy)
    {
        this->setCenterX(xy.x);
        this->setCenterY(xy.y);
    }
    ...
    void setWidth(float w) { this->_w = w; }
    void setHeight(float h) { this->_h = h; }
